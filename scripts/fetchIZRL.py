...
url = "https://ark-funds.com/wp-content/fundsiteliterature/csv/ARK_ISRAEL_INNOVATIVE_TECHNOLOGY_ETF_IZRL_HOLDINGS.csv"

def insert_data(df):
	insert_sql = "INSERT INTO IZRL(date, fund, company, ticker, cusip, shares, value, weight) VALUES "
	row_str_list = []
...
query = conn.execute(f"SELECT COUNT(*) FROM IZRL WHERE date = \"{today.strftime('%-m/%-d/%Y')}\";")
