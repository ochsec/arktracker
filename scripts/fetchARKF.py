...
def insert_data(df):
	insert_sql = "INSERT INTO ARKF(date, fund, company, ticker, cusip, shares, value, weight) VALUES "
	row_str_list = []
...
query = conn.execute(f"SELECT COUNT(*) FROM ARKF WHERE date = \"{today.strftime('%-m/%-d/%Y')}\";")
